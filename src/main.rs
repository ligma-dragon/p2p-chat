...
Iroh is a library for establishing the most direct
QUIC connection possible between two devices.
Every endpoint uses the public half of a cryptographic keypair to identify itself.
Assuming at least one configured relay server is reachable, an endpoint keeps exactly
one TCP connection to a "home relay" that other nodes use for connection establishment,
 and as a fallback transport. Iroh uses a suite of discovery services to resolve home relays & endpoint IDs.
Connections between endpoints use QUIC ALPNs to distinguish between protocols.
Routers automate the endpoint accept loop for protocol multiplexing.
